...
make_mask(hash_t h, __m256i *mask_high, __m256i *mask_low) {
...
inline __attribute__((always_inline)) void bbf_add_key(hash_t h, block_t *data,
                                                       uint32_t num_block) {
    assert((unsigned long)data % 64 == 0);
    uint32_t block_idx = h % num_block;
...
inline __attribute__((always_inline)) _Bool bbf_find(hash_t h, block_t *data,
                                                     uint32_t num_block) {
    assert((unsigned long)data % 64 == 0);
    uint32_t block_idx = h % num_block;
    ...
    make_mask(h, &mask_high, &mask_low);
    __m256i *block = (__m256i *)(data + block_idx);
    return _mm256_testc_si256(*block, mask_high) &&
           _mm256_testc_si256(*(block + 1), mask_low);
}

uint32_t bf_calc_num_blocks(size_t filter_size) {
    assert(filter_size % 64 == 0);
    return filter_size / 64;
}
...
int main() {
    ...
    for (int i = 0; i < KEY_NUM; ++i) {
        ...
        bbf_add_key(hs[i], data, bf_calc_num_blocks(FILTER_SIZE));
    }
    for (int i = 0; i < KEY_NUM; ++i) {
        assert(bbf_find(hs[i], data, bf_calc_num_blocks(FILTER_SIZE)));
    }
    ...
    for (int i = 0; i < 10000; ++i) {
        fp += bbf_find(rand() % 0x10000000 + 0x10000000, data,
                       bf_calc_num_blocks(FILTER_SIZE));
    }
    printf("%d/10000\n", fp);
    assert(fp < 200);
    puts("passed!");
